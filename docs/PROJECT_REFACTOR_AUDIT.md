...
| Full pytest suite | `.\.venv\Scripts\python.exe -m pytest -q` | Windows PowerShell, Python 3.12.12 venv, unrestricted filesystem access | PASS | `215 passed, 1 deselected in 12.04s` at current branch head. |
...
| GitNexus status after latest refresh | `npx gitnexus analyze --force`; `npx gitnexus status` with approved filesystem access | Windows PowerShell, GitNexus CLI via `npx`, full `.git` access | PASS | Forced rebuild completed and status reported matching indexed/current commits with `Status: ✅ up-to-date`. Sandboxed status still cannot read current commit reliably. |
| GitNexus MCP query after latest refresh | `mcp__gitnexus.query("healthz diagnostics runtime")` | Current Codex MCP session | PENDING | Fresh continuation still reported `FTS indexes missing` while CLI status was up-to-date; likely MCP server/session cache or FTS-specific index issue. Rebuild or inspect GitNexus FTS indexes outside this MCP session before relying on keyword ranking. |
